...
module Week05.Learning.Homework1 where
...
mkPolicy pkh deadline () ctx = traceIfFalse "not signed by owner" signedByOwner &&
  ...
  where
    ...
    signedByOwner :: Bool
    signedByOwner = txSignedBy (scriptContextTxInfo ctx) (unPaymentPubKeyHash pkh)
...
test = runEmulatorTraceIO $ do
    ...
    callEndpoint @"mint" h $ MintParams
        { mpTokenName = tn
        , mpDeadline  = (slotToBeginPOSIXTime def 200)
        , mpAmount    = 555
        }
    void $ Emulator.waitNSlots 1
