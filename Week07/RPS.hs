...
transition game s r = case (stateValue s, stateData s, r) of
    ...
    (v, GameDatum _ (Just c), Reveal _ d)
        | lovelaces v == (2 * gStake game) && beats d c  
                                             -> Just ( Constraints.mustBeSignedBy (gFirst game)                     <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)
                                                     , State Finished mempty
                                                     )
        | lovelaces v == (2 * gStake game) && c == d
                                             -> Just ( Constraints.mustBeSignedBy (gFirst game)                                  <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)                    <>
...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just c)) (Reveal nonce d) _ =
    if c == d then
        sha2_256 (nonce `appendByteString` if c == Rock then bsRock' else if c == Paper then bsPaper' else bsScissors') == bs
    else
        sha2_256 (nonce `appendByteString` if c == Rock then bsPaper' else if c == Paper then bsScissors' else bsRock') == bs
...
firstGame fp = do
    ...
    let game   = Game
        ...
        c      = fpChoice fp
        bs     = sha2_256 $ fpNonce fp `appendByteString` if c == Rock then bsRock else if c == Paper then bsPaper else bsScissors
