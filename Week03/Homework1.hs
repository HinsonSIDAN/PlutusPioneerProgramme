...
mkValidator dat () ctx = (traceIfFalse "beneficiary's signature missing" signedByBeneficiary &&
                          traceIfFalse "deadline reached" withinDeadline)||
                         (traceIfFalse "Giver's signature missing" signedByGiver &&
  ...
  where
    ...
    signedByBeneficiary = txSignedBy info $ unPaymentPubKeyHash $ beneficiary1 dat

    withinDeadline :: Bool
    withinDeadline = contains (to $ deadline dat) $ txInfoValidRange info

    signedByGiver :: Bool
    signedByGiver = txSignedBy info $ unPaymentPubKeyHash $ beneficiary2 dat
